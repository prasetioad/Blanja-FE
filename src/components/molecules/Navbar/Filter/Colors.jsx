...
export default function Colors({ col }) {
   // COLOR ARRAY
   const colorArray = [
      "#FFB6C1", "#F08080", "#CD5C5C", "#B0E0E6", "#87CEEB", "#4682B4", "black", 
      "white", "#FFFACD", "#F0E68C", "#FFD700", "#90EE90", "#3CB371", "#2E8B57"
   ]
   // STATE HOOKS
   ...
   <div className={"displayColumn " + css.paddingFilter}>
      ...
      <div className={"displayRow " + css.filterOption + " " + css.filterColorOption}>
         {
         colorArray.length < 8 ? 
         null
         :
         ...
         {colorArray.slice(startNum, endNum).map((item) => 
            <div className={col[0] === item ? css.colorBorderActive : css.colorBtnBorder} color={item} onClick={ col[1] }>
               <Button 
                  btnClr={item} 
                  cls={
                     item === "white" ? "hoverThis " + css.whiteColorBtn 
                     : 
